...
| File visitors download from the site | `site/downloads/AudiobookBinder.dmg` (that same DMG) |
...
4. `make release` — builds the signed DMG, creates or updates GitHub tag `vX.Y.Z`, copies that DMG to `site/downloads/AudiobookBinder.dmg`, sets `PUBLIC_VERSION`.
